...
<body>
	...
	<div class="jumbotron">
		<div class="container">
			<div class="transbox">
				<h1><?php if(isset($_SESSION['login_MEMBERID'])) print 'Welcome '.$_SESSION['login_FIRSTNAME'].". ";?>Find a place to stay in Lindbergh.</h1>
		...
		<div class="row">
			<div class="col-md-4 col-sm-6">
				...
				<?php
					...
					if(isset($_SESSION['login_MEMBERID']) && $_SESSION['login_USERTYPE'] == 'owner') {
						...
						foreach($ages as $age) {
							$tenant_sql = "SELECT * FROM tenantprofiles WHERE age < $age";

							if($i > 0) {
								$tenant_sql .= " AND age >= ".$ages[$i-1];
							}

							$tenant_result = mysqli_query($connection, $tenant_sql);

							$number_of_tenants = mysqli_num_rows($tenant_result);
							echo "<p>Number of tenants who are under $age: ".$number_of_tenants."</p>";
							$i++;
						}
						// $tenant_sql = "SELECT * FROM tenantprofiles";
						// $tenant_result = mysqli_query($connection, $tenant_sql);

						// $number_of_tenants = mysqli_num_rows($tenant_result);
						// echo "<br><p>Number of tenants who updated their profiles are: </p>".$number_of_tenants;
					}
		...
		<div class="container">
			<div class="row">
				<div class="col-md-10  col-md-offset-1 text-center">
					<h3>Contact</h3>
					<p><small> <strong>Joshitha Velkur, Srinu Gopathi</strong><br>
						1234 Sidney Marcus Blvd NE <br>
						Atlanta, Georgia, USA <br>
						</small> </p>
					</div>
				</div>
				<p class="text-center col-md-12"r style="padding-top: 30px">
					<small>© Copyright Joshitha Velkur, Srinu Gopathi. All rights reserved.</small>
				</p>
			</div>
		</footer>
